...
def withdraw(user, amount):
    ...
    if profile.balance >= amount and amount > 0:
...
def transfer(sender, recipient, amount):
    ...
    amount = Decimal(amount)
    if sender_profile.balance >= amount and amount > 0:
        sender_profile.balance -= amount
        recipient_profile.balance += amount
        sender_profile.save()
        recipient_profile.save()
        Transaction.objects.create(user=sender, transaction_type='transfer', amount=amount, to_user=recipient)
        return True
    return False
